...
contract Arbitration is ReentrancyGuard {
    ...
    event JurorStaked(address indexed juror, uint256 amount, string[] categories);
    event JurorUnstaked(address indexed juror, uint256 amount);
    event DisputeCreated(uint256 indexed disputeId, uint256 indexed jobId, string category);
    ...
    event DisputeResolved(uint256 indexed disputeId, bool clientWon, uint256 majorityVotes);
    event DisputeAppealed(uint256 indexed disputeId, uint256 newJurorCount);
    event RewardsDistributed(uint256 indexed disputeId, address[] winners, uint256 rewardPerJuror);
    ...
    function stakeAsJuror(string[] memory _categories) external nonReentrant {
        ...
        require(
            arbitrationToken.transferFrom(msg.sender, address(this), STAKE_AMOUNT),
    ...
    function createDispute(
        ...
        require(jurorsByCategory[_category].length >= INITIAL_JUROR_COUNT, "Not enough jurors");
     ...
     */
    function vote(uint256 _disputeId, bool _voteForClient) external {
        ...
        if (dispute.votesForClient + dispute.votesForWorker == dispute.jurorCount) {
    ...
    function appealDispute(uint256 _disputeId) external payable {
        ...
        for (uint256 i = 0; i < dispute.selectedJurors.length; i++) {
            dispute.hasVoted[dispute.selectedJurors[i]] = false;
        }
     ...
     */
    function getDisputeResult(uint256 _disputeId) 
        external 
        view 
        returns (bool isResolved, bool clientWon) 
    {
        Dispute storage dispute = disputes[_disputeId];
        return (dispute.isResolved, dispute.votesForClient > dispute.votesForWorker);
    }
    ...
    function _selectJurors(uint256 _disputeId, uint256 _count) internal {
        ...
        uint256 startIndex = uint256(keccak256(abi.encodePacked(
            block.timestamp,
            block.prevrandao,
            _disputeId
        ...
        while (selected < _count && selected < categoryJurors.length) {
            ...
            if (jurors[juror].isStaked && !_isSelectedJuror(_disputeId, juror)) {
    ...
    function _resolveDispute(uint256 _disputeId) internal {
        ...
        uint256 majorityVotes = clientWon ? dispute.votesForClient : dispute.votesForWorker;
    ...
    function _distributeRewards(uint256 _disputeId, bool _clientWon) internal {
        ...
        for (uint256 i = 0; i < dispute.selectedJurors.length; i++) {
            address juror = dispute.selectedJurors[i];
            if (dispute.hasVoted[juror] && dispute.votedForClient[juror] == _clientWon) {
     ...
     */
    function _slashLosers(uint256 _disputeId, bool _clientWon) internal returns (uint256) {
        Dispute storage dispute = disputes[_disputeId];
     ...
     */
    function _removeJurorFromCategory(address _juror, string memory _category) internal {
     ...
     */
    function getJurorCategories(address _juror) external view returns (string[] memory) {
     ...
     */
    function getJurorCountByCategory(string memory _category) external view returns (uint256) {
