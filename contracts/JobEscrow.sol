...
contract JobEscrow is ReentrancyGuard {
    ...
    event WorkerSet(address indexed worker);
    event DisputeRaised();
    event EscrowCompleted();

    modifier onlyClientOrMarketplace() {
        require(
            msg.sender == client || msg.sender == marketplace,
            "Only client or marketplace"
        );
        _;
    }
        ...
        nonReentrant
    {
        ...
        require(usdc.transfer(worker, amount), "USDC transfer failed");
    ...
    function refundClient(
        uint256 _amount
    ) external onlyMarketplace onlyWhenDisputed notCompleted nonReentrant {
        require(isFunded, "Not funded");
        ...
        // Interactions - transfer USDC
        require(usdc.transfer(client, _amount), "USDC refund failed");
    ...
    function payWorker(
        ...
        // Interactions - transfer USDC
        require(usdc.transfer(worker, _amount), "USDC payment failed");
    ...
    /**
     * @dev Cancel and refund client before any worker selection
     */
        ...
        nonReentrant
    {
        ...
        require(usdc.transfer(client, amount), "USDC refund failed");

        emit RefundIssued(client, amount);
        emit EscrowCompleted();
    }
