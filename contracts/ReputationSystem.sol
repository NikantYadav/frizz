...
contract ReputationSystem is Ownable {
    ...
    /**
     * @dev Set marketplace address (only once, by owner)
     * This is the primary authorized caller for reputation updates
     */
    ...
    /**
     * @dev Authorize additional contracts to update reputation
     */
    ...
    function getJobRecord(
        uint256 _jobId
    ) external view returns (JobRecord memory) {
        return jobs[_jobId];
    }
     ...
     */
    function areBothRatingsComplete(
        uint256 _jobId
    ) external view returns (bool) {
        JobRecord storage job = jobs[_jobId];
